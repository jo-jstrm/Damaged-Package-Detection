...
Based on jkjung: https://github.com/jkjung-avt/tensorrt_demos
...
def load_plugin_layer():
    ...
    try:
        ...
        raise SystemExit('ERROR: failed to load libyolo_layer.so.  '
...
class TrtYOLO(Model):
    ...
    def _load_engine(self):
        TRTbin = self.model
        print(f'Trying to open: {TRTbin}')
        with open(TRTbin, 'rb') as f, trt.Runtime(self.trt_logger) as runtime:
            return runtime.deserialize_cuda_engine(f.read())

    def __init__(self, model_path, category_num=80, letter_box=False, cuda_ctx=None):
        """Initialize TensorRT plugins, engine and conetxt."""
        Model.__init__(self, model_path, 1)
